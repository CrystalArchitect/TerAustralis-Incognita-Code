...
<script>
  ...
  // The storage keys changed with the rename. A person's voice settings are
  // theirs; read the old keys when the new ones are absent so nobody's
  // preference is silently reset on upgrade.
  const stored = (key) =>
    localStorage.getItem(`clementine.${key}`) ?? localStorage.getItem(`lumina.${key}`);

  let voiceOn = $state(stored('voice') === 'on');
  let voiceName = $state(stored('voiceName') || '');
