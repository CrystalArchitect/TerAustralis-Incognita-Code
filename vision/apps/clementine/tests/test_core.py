...
Run: `python -m pytest vision/apps/clementine/tests`
"""
...
from crystalcore.mind.memory import Personality
from crystalcore.mind import profiles
...
def test_cosine_identical_is_one():
    assert CrystalCore._cosine([1.0, 2.0, 3.0], [1.0, 2.0, 3.0]) == pytest.approx(1.0)


def test_cosine_orthogonal_is_zero():
    assert CrystalCore._cosine([1.0, 0.0], [0.0, 1.0]) == pytest.approx(0.0)


def test_cosine_zero_vector_is_zero_not_error():
    # A zero-norm vector must not raise ZeroDivisionError.
    assert CrystalCore._cosine([0.0, 0.0], [1.0, 2.0]) == 0.0


def test_recency_factor_now_is_full_weight():
    from datetime import datetime
    now = datetime.now().isoformat()
    assert CrystalCore._recency_factor(now) == pytest.approx(1.0, abs=0.01)


def test_recency_factor_floor_is_point_seven():
    from datetime import datetime, timedelta
    old = (datetime.now() - timedelta(days=800)).isoformat()  # past the 365 cap
    assert CrystalCore._recency_factor(old) == pytest.approx(0.7)


def test_recency_factor_bad_stamp_defaults_to_full_weight():
    assert CrystalCore._recency_factor("not-a-date") == 1.0
    assert CrystalCore._recency_factor(None) == 1.0


def test_split_tags_extracts_trailing_hashtags():
    clean, tags = CrystalCore._split_tags("loves the night sky #family #stars")
    assert clean == "loves the night sky"
    assert tags == ["family", "stars"]


def test_split_tags_none_present():
    clean, tags = CrystalCore._split_tags("just a plain memory")
...
def test_save_load_round_trip(tmp_path):
    c = _offline(CrystalCore(memory_dir=str(tmp_path)))
    c.remember("keeps a telescope on the balcony #stars")
    c.remember_fact("home_city", "Perth")

    reloaded = _offline(CrystalCore(memory_dir=str(tmp_path)))
...
def test_corrupt_file_is_preserved_never_deleted(tmp_path):
    ...
    c = CrystalCore(memory_dir=str(tmp_path))  # __init__ calls load()
...
def test_unknown_fields_are_ignored(tmp_path):
    ...
    c = CrystalCore(memory_dir=str(tmp_path))
...
def test_condense_noop_at_or_below_limit(tmp_path, monkeypatch):
    c = _offline(CrystalCore(memory_dir=str(tmp_path), max_recent_turns=2))  # limit = 4
